...
PoissonSolver_p2::PoissonSolver_p2(const TaskParams& config,
...
// ПЕРЕИМЕНОВАНО
void PoissonSolver_p2::init_grid() {
    // Граничные условия
    for (int i = 0; i <= cfg.n; ++i) {
...
// ПЕРЕИМЕНОВАНО
double PoissonSolver_p2::calculate_max_error() const {
    double max_err = 0.0;
    double hx = (cfg.b - cfg.a) / cfg.n;
    double hy = (cfg.d - cfg.c) / cfg.m;
    for (int i = 0; i <= cfg.n; ++i) {
